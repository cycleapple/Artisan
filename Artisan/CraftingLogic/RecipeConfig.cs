...
public class RecipeConfig
{
    ...
    public string PotionName => requiredPotion == Default ? $"{P.Config.DefaultConsumables.PotionName}（預設）" : RequiredPotion == Disabled ? "已停用" : $"{(RequiredPotionHQ ? " " : "")}{ConsumableChecker.Pots.FirstOrDefault(x => x.Id == RequiredPotion).Name}";
    public string ManualName => requiredManual == Default ? $"{P.Config.DefaultConsumables.ManualName}（預設）" : RequiredManual == Disabled ? "已停用" : $"{ConsumableChecker.Manuals.FirstOrDefault(x => x.Id == RequiredManual).Name}";
    public string SquadronManualName => requiredSquadronManual == Default ? $"{P.Config.DefaultConsumables.SquadronManualName}（預設）" : RequiredSquadronManual == Disabled ? "已停用" : $"{ConsumableChecker.SquadronManuals.FirstOrDefault(x => x.Id == RequiredSquadronManual).Name}";
    ...
    public bool DrawSolver(CraftState craft, bool hasButton = false, bool liveStats = true)
    {
        ...
        if (ImGui.BeginCombo("##solver", solver.Name))
        {
            foreach (var opt in CraftingProcessor.GetAvailableSolversForRecipe(craft, true))
            {
                ...
                {
                    ImGui.Text($"不支援 {opt.Name}－{opt.UnsupportedReason}");
                }
                else
                {
        ...
        if (!P.Config.HideRecipeWindowSimulator)
        {
            ...
            if (solver.Name != "Expert Recipe Solver")
            {
                if (craft.MissionHasMaterialMiracle && solver.Name == "Standard Recipe Solver" && P.Config.UseMaterialMiracle)
                    ImGuiEx.TextWrapped("此設定會使用「素材奇蹟」，目前不相容於模擬器。");
                else
                if (solver.Name == "Raphael Recipe Solver" && !RaphaelCache.HasSolution(craft, out _))
                    ImGuiEx.TextWrapped("尚未產生 Raphael 解法，無法建立模擬。");
                else
                    ImGuiEx.TextWrapped(hintColor, solverHint);
            }
            else
                ImGuiEx.TextWrapped("請在模擬器中執行此配方以查看結果。");
