...
namespace Artisan.FCWorkshops
{
    internal static class FCWorkshopUI
    {
        ...
        internal static void Draw()
        {
            ...
            ImGui.TextWrapped("此分頁可瀏覽遊戲中的所有部隊工房工程。內容分為三個主要區段：完整工程總覽、各零件明細，以及各製作階段。每個區段都可建立包含該部分所需材料的製作清單。");
            ...
            if (SelectedProject != 0)
            {
                ...
                if (ImGui.CollapsingHeader("工程資訊"))
                {
                    ...
                    if (ImGui.BeginTable($"###FCWorkshopProjectItemsContainer", RetainerInfo.ATools ? 4 : 3, ImGuiTableFlags.Borders))
                    {
                        ...
                        ImGui.TableSetupColumn("物品欄", ImGuiTableColumnFlags.WidthFixed);
                        if (RetainerInfo.ATools) ImGui.TableSetupColumn("雇員", ImGuiTableColumnFlags.WidthFixed);
                    ...
                    if (ImGui.Button("為此工程建立製作清單", new Vector2(ImGui.GetContentRegionAvail().X, 24f.Scale())))
                    {
                        Notify.Info("正在建立清單，請稍候。");
                        Task.Run(() => CreateProjectList(project, false)).ContinueWith((_) => Notify.Success("部隊工房清單已建立"));
                    }

                    if (ImGui.Button("為此工程建立製作清單（包含半成品）", new Vector2(ImGui.GetContentRegionAvail().X, 24f.Scale())))
                    {
                        Notify.Info("正在建立清單，請稍候。");
                        Task.Run(() => CreateProjectList(project, true)).ContinueWith((_) => Notify.Success("部隊工房清單已建立"));
                    }
                }
                if (ImGui.CollapsingHeader("工程零件"))
                {
                    ImGui.Indent();
                    string partNum = "";
                    foreach (var part in project.CompanyCraftPart.Where(x => x.RowId > 0).Select(x => x.Value))
                    {
                        ...
                        if (ImGui.CollapsingHeader($"{partNum}"))
                        {
                            ...
                            if (ImGui.Button("為此零件建立製作清單", new Vector2(ImGui.GetContentRegionAvail().X, 24f.Scale())))
                            {
                                Notify.Info("正在建立清單，請稍候。");
                                Task.Run(() => CreatePartList(part, partNum, false)).ContinueWith((_) => Notify.Success("部隊工房清單已建立"));
                            }

                            if (ImGui.Button("為此零件建立製作清單（包含半成品）", new Vector2(ImGui.GetContentRegionAvail().X, 24f.Scale())))
                            {
                                Notify.Info("正在建立清單，請稍候。");
                ...
                if (ImGui.CollapsingHeader("工程階段"))
                {
                    string pNum = "";
                    foreach (var part in project.CompanyCraftPart.Where(x => x.RowId > 0).Select(x => x.Value))
                    {
                        ...
                        {
                        if (ImGui.CollapsingHeader($"{pNum}－階段 {phaseNum}"))
                            {
                                ...
                                ImGui.TableSetupColumn("所需組數", ImGuiTableColumnFlags.WidthFixed);
                                ImGui.TableSetupColumn("需求總數", ImGuiTableColumnFlags.WidthFixed);
                                ImGui.TableSetupColumn("物品欄", ImGuiTableColumnFlags.WidthFixed);
                                if (RetainerInfo.ATools) ImGui.TableSetupColumn("雇員", ImGuiTableColumnFlags.WidthFixed);
                                    ImGui.TableHeadersRow();
                                ...
                                if (ImGui.Button($"為此階段建立製作清單（包含半成品）###PhaseButtonPC{phaseNum}", new Vector2(ImGui.GetContentRegionAvail().X, 24f.Scale())))
                                {
                                    Notify.Info("正在建立清單，請稍候。");
